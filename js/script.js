...
const hideElem = (elem, cb) => {
    // так как у нас нет прямого доступа к css св-ам мы можем получить только те css св-ва, которые уже высчитал браузер
    ...
    // эта ф-ия, которая будет запускать другую ф-ию в тот момент когда у нас обновляеться экран
    requestAnimationFrame(animation);
}
...
const renderTheme = themes => {
    ...
    // 2-ой вариант без создания дополнительной переменной и пустого массива
...
const renderQuiz = quiz => {
    ...
    const showQuestion = () => {
        ...
        form.addEventListener('submit', event => {
            ...
            let ok = false;
            // Получаем все радиокнопки и чекбоксы. Преобразуем html коллекцию в обычный массив
            ...
            if (ok) {
                // метод every проверяет каждый элемент и если все значения true, т.е когда выбраны все правильные ответы, то тогда в r запишеться значение true
                // если хоть одно условие вернёт false, то этот метод вернёт false
                // оператор !! преобразует в булевское значение
...
const addClick = (buttons, data) => {
    buttons.forEach(btn => {
        btn.addEventListener('click', () => {
            // Проверка соответствия id кнопки и id базы данных. Если они совпали, то этот объект возвращаеться в переменную quiz
